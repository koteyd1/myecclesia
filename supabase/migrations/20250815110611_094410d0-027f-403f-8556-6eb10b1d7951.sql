-- Security fix for donations table: Protect anonymous donor PII

-- 1. First, let's drop existing problematic policies and create more secure ones
DROP POLICY IF EXISTS "comprehensive_donations_select" ON public.donations;
DROP POLICY IF EXISTS "comprehensive_donations_update" ON public.donations;
DROP POLICY IF EXISTS "donations_update_access" ON public.donations;

-- 2. Create secure view function for admin access to sensitive data
...
-- 3. Create secure view function for user's own donations (masked sensitive data)
...
-- Only admins can view all donations (including anonymous ones)
CREATE POLICY "admins_can_view_all_donations" ON public.donations
FOR SELECT 
USING (is_admin_user());

-- Authenticated users can only view their own non-anonymous donations
CREATE POLICY "users_view_own_donations_only" ON public.donations
FOR SELECT 
USING (
...
-- More restrictive update policy - only admins or donation owners
...
CREATE OR REPLACE FUNCTION public.audit_sensitive_donation_access()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF (OLD.user_id IS NULL OR NEW.user_id IS NULL) OR 
     (OLD.email IS NOT NULL OR NEW.email IS NOT NULL) OR
     (OLD.stripe_customer_id IS NOT NULL OR NEW.stripe_customer_id IS NOT NULL) THEN
    ...
    INSERT INTO public.admin_audit_log (
      ...
      auth.uid(),
      CASE 
        WHEN (OLD.user_id IS NULL OR NEW.user_id IS NULL) THEN 
...
CREATE TRIGGER audit_sensitive_donation_access_trigger
  AFTER SELECT ON public.donations
  FOR EACH ROW
  EXECUTE FUNCTION public.audit_sensitive_donation_access();

-- 7. Create function to mask sensitive payment data for non-admin users
...
-- 8. Create function to validate anonymous donation data before insertion
...
-- 9. Create trigger for anonymous donation validation
...
CREATE TRIGGER validate_anonymous_donation_trigger
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.validate_anonymous_donation_security();